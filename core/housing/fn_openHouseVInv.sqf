...
createDialog "life_virtInv_house";
...
_index = -1;
{
	if (_x select 0 != "B_supplyCrate_F" AND _x select 0 != "Box_IND_AmmoVeh_F" AND _x select 0 != "Box_IND_WpsSpecial_F" AND _x select 0 != "Box_IND_Grenades_F" AND _x select 0 != "_className") then {
		...
		if (_x select 0 == "B_supplyCrate_F") then {_boxslots = _boxslots + 500;};
		if (_x select 0 == "Box_IND_AmmoVeh_F") then {_boxslots = _boxslots + 500;};
		if (_x select 0 == "Box_IND_WpsSpecial_F") then {_boxslots = _boxslots + 250;};
		if (_x select 0 == "Box_IND_Grenades_F") then {_boxslots = _boxslots + 150;};
	};
} forEach _content;
