...
if(_curTarget isKindOf "House_F") exitWith {
	if((nearestObject [[1552.1,11980.5,0],"Land_Dome_Big_F"]) == _curTarget OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _curTarget) then {
...
if(!((player distance (getMarkerPos "jailrange") < 100) OR  (player distance (getMarkerPos "police_hq_x") < 30) OR (player distance (getMarkerPos "cop_spawn_x") < 30) OR (player distance (getMarkerPos "cop_spawn_x") < 30))) then 
{
